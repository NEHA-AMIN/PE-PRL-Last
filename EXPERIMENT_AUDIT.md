...
**Audit Coverage:** Baseline + 7 ablation experiments (fully read), 4 Formula-variant experiments (structure confirmed, model files not read in detail).
...
**Experiments NOT fully audited (model files not read):**
- Exp3b (E-96-3b-Label-Temporal-Controlled) — structure confirmed
- Formula-A-pos, Formula-B-pos, Formula-B-sem, formula-A-sem — not read
- Archived experiments — not read (archived)
...
**Average implementation confidence: 8.2/10** — Code quality is generally high and formulas are correctly implemented.  
**Average documentation confidence: 3.2/10** — Documentation contains multiple errors and outdated values.  
**Average results confidence: 3.0/10** — Central results cannot be verified; key comparison values in README appear incorrect or unlogged.
...
4. **Fix Exp6-Post encoder** to downsample `delta_x` through `ConvLayer` (matching Exp5b)
5. **Fix Exp1 README** `Prediction Length: 24` → 48/96/192/336
6. **Fix baseline_phase1.sh** `Informer2020` → `Informer2020-original`
7. **Add Exp6-Pre shell scripts** (missing from directory)

### Priority 3 — Methodology
8. **Create controlled baseline** with `--attn full`, `--embed fixed`, `--d_ff 512` for clean comparison
9. **Document decoder cross-attention decay** as a known confound
10. **Note in paper** that Exp5's Label and Order signals are algebraically dependent (O_i = P_i − P̄)
