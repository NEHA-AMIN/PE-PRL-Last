#!/bin/bash
# =============================================================================
# Phase 2: Experiment 1 POST SOFTMAX on ETTh1 (Alpha = 0.5) — FORCE RE-RUN
# Forces re-run of: seed=2023, pred_len=192 and 336 (no skip logic)
# =============================================================================
...
mkdir -p "$LOG_DIR"
MASTER_LOG="$LOG_DIR/master_run_alpha0.5_rerun.log"

echo "============================================================" | tee "$MASTER_LOG"
echo "Exp1-Post  |  PHASE 2 FORCE RE-RUN (alpha=0.5)"               | tee -a "$MASTER_LOG"
echo "============================================================" | tee -a "$MASTER_LOG"
echo "Forcing re-run: seed=2023, pred_len=192 & 336"                | tee -a "$MASTER_LOG"
...
for PRED_LEN in 192 336; do
    ...
    # Preserve previous log if it exists
    if [ -f "$RUN_LOG" ]; then
        TS=$(date +%Y%m%d_%H%M%S)
        mv "$RUN_LOG" "${RUN_LOG}.old_${TS}"
        echo "Renamed old log to ${RUN_LOG}.old_${TS}" | tee -a "$MASTER_LOG"
    fi
...
# Note: skipping `git checkout ./models/` since the directory isn't a git repo
...
echo "" | tee -a "$MASTER_LOG"
echo "============================================================" | tee -a "$MASTER_LOG"
echo "RE-RUN COMPLETE — RESULTS SUMMARY (alpha=$ALPHA)"             | tee -a "$MASTER_LOG"
echo "Total: $TOTAL | Done: $COMPLETED | Failed: $FAILED"           | tee -a "$MASTER_LOG"
echo "End time: $(date)" | tee -a "$MASTER_LOG"
echo "============================================================" | tee -a "$MASTER_LOG"
echo "" | tee -a "$MASTER_LOG"
echo "Per-run MSE/MAE (alpha=$ALPHA, post-softmax) — re-run results:" | tee -a "$MASTER_LOG"
