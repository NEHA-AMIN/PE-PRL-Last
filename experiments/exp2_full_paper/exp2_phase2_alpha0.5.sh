#!/bin/bash
# =============================================================================
# Phase 2: Experiment 2 Full LOD on ETTh1 (Alpha = 0.5)
# Runs:     2 (pred_len 192, 336) × 3 (seed 2021, 2022, 2023)
# =============================================================================

PROJECT_ROOT="/content/drive/MyDrive/Dist-Abl-PRL-All-Exs-ETTH1"
INFORMER_DIR="$PROJECT_ROOT/Informer2020-original"
LOG_DIR="$PROJECT_ROOT/logs/exp2_phase2"
...
echo "============================================================" | tee "$MASTER_LOG"
echo "Exp2-Full  |  PHASE 2 — RESULTS STABILITY (alpha=0.5)" | tee -a "$MASTER_LOG"
echo "============================================================" | tee -a "$MASTER_LOG"
echo "Seed      : 2021, 2022, 2023 | Pred lengths: 192, 336"    | tee -a "$MASTER_LOG"
...
cp -r ../experiments/exp2_full_paper/models/* ./models/
...
# Modify the decay_a parameter in the copied embed.py
...
for SEED in 2021 2022 2023; do
    for PRED_LEN in 192 336; do
        TOTAL=$((TOTAL + 1))
        RUN_ID="exp2_ph2_ETTh1_alpha${ALPHA}_pred${PRED_LEN}_seed${SEED}"
...
echo "============================================================" | tee -a "$MASTER_LOG"
echo "" | tee -a "$MASTER_LOG"
echo "Per-run MSE/MAE (alpha=$ALPHA, Exp2 LOD):" | tee -a "$MASTER_LOG"
