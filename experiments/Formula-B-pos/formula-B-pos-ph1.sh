#!/bin/bash
# =============================================================================
# Formula B (Positional Space) — Phase 1: Global Mean Deviation Ordering
#
# Experiment name : Formula-B-pos
# Embedding class : DataEmbedding_formula_b_pos
# pe_mode flag    : --pe_mode formula_b_pos
#
# Formula
# -------
#   Output:   X'_i = V_i + T_i + P_i + Ordering_i
#
#   where:
#     V_i       = TokenEmbedding(x)                     [B, L, D]
#     P_i       = LegendrePositionEmbedding(x)          [B, L, D]
#     mu_p      = (1/L) * sum_k P_k                     [B, 1, D]  global mean of P
#     delta_p_i = mu_p - P_i                            [B, L, D]
#     p_bar     = (1/L) * sum_i ||P_i||_2               scalar normaliser
#     Ordering_i = delta_p_i / (p_bar + 1e-8)           [B, L, D]
#
# Contrast with Formula-A-pos
# ---------------------------
#   Formula A (positional): delta_p_i = P_i - P_{i-1}      consecutive diff
#   Formula B (positional): delta_p_i = mu_p - P_i          global mean deviation
#
# What changed from Formula-A-pos
# --------------------------------
#   CHANGED : delta_p computation (2 lines in embed.py)
#   CHANGED : pe_mode dispatch string ('formula_b_pos' vs 'ordering_pos')
#   UNCHANGED: TokenEmbedding, TemporalEmbedding, LegendrePositionEmbedding,
#              p_bar normalisation, Encoder, Decoder, Attention, projection head,
#              all hyperparameters
#
...
#   Establish whether global-mean-deviation ordering in positional (Legendre)
...
EXP_DIR="$PROJECT_ROOT/experiments/Formula-B-pos"
LOG_DIR="$PROJECT_ROOT/logs/Formula-B-pos-phase1"
...
{
echo "============================================================"
echo " Formula B (Positional Space) — Phase 1: Legendre Global Mean Deviation"
echo " Date: $(date)"
echo "------------------------------------------------------------"
echo " Formula:  X'_i = V_i + T_i + P_i + Ordering_i"
echo " Signal:   Ordering_i = (mu_p - P_i) / (p_bar + 1e-8)"
echo " pe_mode:  formula_b_pos"
echo "------------------------------------------------------------"
echo " Seed:       2021"
...
for pred_len in 96 192; do
    ...
    TOTAL=$((TOTAL + 1))
    RUN_ID="formula_b_pos_ph1_ETTh1_pred${pred_len}_seed${SEED}"
...
# --- SUMMARY ---
echo "============================================================" | tee -a "$MASTER_LOG"
echo " PHASE 1 SUMMARY — Formula B (Positional Space)"            | tee -a "$MASTER_LOG"
...
echo "   Formula-B-pos wins BOTH vs Exp1-Pre → proceed to Phase 2" | tee -a "$MASTER_LOG"
echo "   Formula-B-pos wins ONE  vs Exp1-Pre → Phase 2 to confirm" | tee -a "$MASTER_LOG"
echo "   Formula-B-pos loses BOTH            → document, stop"     | tee -a "$MASTER_LOG"
echo "============================================================" | tee -a "$MASTER_LOG"
echo "End: $(date)" | tee -a "$MASTER_LOG"
