...
#   at short and mid horizon before committing to full Phase 2 sweep.
#
...
EXP_DIR="$PROJECT_ROOT/experiments/exp6_lod_pre"
LOG_DIR="$PROJECT_ROOT/logs/exp6_lod_pre_phase1"
...
{
echo "============================================================"
echo " Experiment 6 LOD Pre-Softmax — Phase 1: Alpha Screening"
echo " Date: $(date)"
echo "------------------------------------------------------------"
echo " Architecture:"
echo "   Q, K  ← combined_emb  (value + temporal + legendre)"
echo "   V     ← delta_x       (x_i - x_{i-1})"
echo "   alpha = 1/(1 + |i-j|^decay_a)  applied BEFORE softmax"
echo "------------------------------------------------------------"
...
for DECAY_A in 0.5 1.0 2.0; do
    for pred_len in 96 192; do
        ...
        TOTAL=$((TOTAL + 1))
        RUN_ID="exp6pre_ph1_ETTh1_lod_pre_a${DECAY_A}_pred${pred_len}_seed${SEED}"
...
# --- SUMMARY TABLE ---
echo "============================================================" | tee -a "$MASTER_LOG"
echo " PHASE 1 SUMMARY — Experiment 6 LOD Pre-Softmax"            | tee -a "$MASTER_LOG"
...
echo ""                                                            | tee -a "$MASTER_LOG"
echo " Step 1 — Which alpha wins?"                                 | tee -a "$MASTER_LOG"
echo "   Compare MSE across decay_a in {0.5, 1.0, 2.0} at each pred_len." | tee -a "$MASTER_LOG"
echo "   Smaller decay_a (0.5) → gentler falloff (distant tokens less penalised)" | tee -a "$MASTER_LOG"
echo "   Larger decay_a (2.0)  → steeper falloff (distant tokens strongly suppressed)" | tee -a "$MASTER_LOG"
echo ""                                                            | tee -a "$MASTER_LOG"
echo " Step 2 — Does best alpha beat Exp5b (no distance)?"        | tee -a "$MASTER_LOG"
echo "   YES → Distance pre-softmax adds value. Use best alpha in Phase 2." | tee -a "$MASTER_LOG"
echo "   NO  → Document as negative: L+O clean delta alone is better." | tee -a "$MASTER_LOG"
echo ""                                                            | tee -a "$MASTER_LOG"
echo " Step 3 — Does best alpha beat Exp1-Pre (distance only)?"   | tee -a "$MASTER_LOG"
echo "   YES → Full LOD combination improves over distance alone." | tee -a "$MASTER_LOG"
echo "   NO  → delta-V component is hurting, not helping."        | tee -a "$MASTER_LOG"
