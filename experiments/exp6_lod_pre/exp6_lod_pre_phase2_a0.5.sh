...
#
# Phase 2 goal:
#   Stability validation — fix decay_a=0.5 (best alpha for pred=192 from
...
LOG_DIR="$PROJECT_ROOT/logs/exp6_lod_pre_phase2_a0.5"
...
{
echo "============================================================"
echo " Experiment 6 LOD Pre-Softmax — Phase 2: Stability Validation (decay_a=0.5)"
echo " Date: $(date)"
echo "------------------------------------------------------------"
echo " Architecture:"
...
echo "------------------------------------------------------------"
echo " decay_a fixed at 0.5 (best alpha for pred=192 from Phase 1)"
...
echo "   decay_a=0.5: pred_96  MSE=0.9849  MAE=0.8068"
echo "   decay_a=0.5: pred_192 MSE=0.9075  MAE=0.7574  ← best at pred=192"
echo "   (decay_a=2.0 was best at pred_96: MSE=0.9629)"
echo "============================================================"
} | tee "$MASTER_LOG"
...
for pred_len in 48 96 192 336; do
    for SEED in 2021 2022 2023; do
        ...
        TOTAL=$((TOTAL + 1))
        RUN_ID="exp6pre_ph2_ETTh1_lod_pre_a0.5_pred${pred_len}_seed${SEED}"
...
# --- SUMMARY TABLE ---
echo "============================================================" | tee -a "$MASTER_LOG"
echo " PHASE 2 SUMMARY — Experiment 6 LOD Pre-Softmax (decay_a=0.5)" | tee -a "$MASTER_LOG"
