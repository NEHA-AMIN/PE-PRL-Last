...
#
# Phase 2 goal:
#   Stability validation — fix decay_a=2.0 (best alpha for pred=96 from
#   Phase 1) and sweep all pred_lens across 3 seeds to confirm reproducibility.
#
...
LOG_DIR="$PROJECT_ROOT/logs/exp6_lod_pre_phase2_a2.0"
...
{
echo "============================================================"
echo " Experiment 6 LOD Pre-Softmax — Phase 2: Stability Validation (decay_a=2.0)"
echo " Date: $(date)"
echo "------------------------------------------------------------"
echo " Architecture:"
...
echo "------------------------------------------------------------"
echo " decay_a fixed at 2.0 (best alpha for pred=96 from Phase 1)"
echo " Pred lens:     48, 96, 192, 336"
echo " Seeds:         2021, 2022, 2023"
echo " Total runs:    12  (4 pred_lens × 3 seeds)"
echo "------------------------------------------------------------"
echo " Phase 1 results (seed=2021):"
echo "   decay_a=2.0: pred_96  MSE=0.9629  MAE=0.7839"
echo "   decay_a=2.0: pred_192 MSE=0.9780  MAE=0.7907"
echo "   (decay_a=0.5 was best at pred_192: MSE=0.9075)"
echo "============================================================"
} | tee "$MASTER_LOG"
...
# --- LOOPS ---
# Phase 2: decay_a=2.0 fixed; outer=pred_len, inner=seed

for pred_len in 48 96 192 336; do
    for SEED in 2021 2022 2023; do

        TOTAL=$((TOTAL + 1))
        RUN_ID="exp6pre_ph2_ETTh1_lod_pre_a2.0_pred${pred_len}_seed${SEED}"
        RUN_LOG="$LOG_DIR/${RUN_ID}.log"
        mkdir -p "$RESULTS_DIR/${RUN_ID}"

        echo "------------------------------------------------------------" | tee -a "$MASTER_LOG"
        echo "RUN $TOTAL/12: $RUN_ID" | tee -a "$MASTER_LOG"
        echo "pred_len=$pred_len | seed=$SEED | decay_a=2.0" | tee -a "$MASTER_LOG"
...
# --- SUMMARY TABLE ---
echo "============================================================" | tee -a "$MASTER_LOG"
echo " PHASE 2 SUMMARY — Experiment 6 LOD Pre-Softmax (decay_a=2.0)" | tee -a "$MASTER_LOG"
