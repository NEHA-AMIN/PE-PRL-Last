...
class DataEmbedding(nn.Module):
    def __init__(self, c_in, d_model, embed_type='fixed', freq='h', dropout=0.1):
        ...
        self.legendre_embedding = LegendrePositionEmbedding(d_model=d_model)
    ...
    def forward(self, x, x_mark):
        ...
        # CLEAN DELTA: Δx[i] = x_i - x_{i-1}, computed from value_emb ONLY
        # NO p_i terms, NO T_i terms
        ...
        # p_i: Legendre position embedding (label)
        legendre_pos = self.legendre_embedding(x)
        
        # Combined embedding for Q/K: x_i + T_i + p_i
        combined_emb = value_emb + temporal_emb + legendre_pos
        
        # Return tuple: (combined_emb, delta_x)
        # combined_emb will be used for Q and K
        # delta_x will be used for V
