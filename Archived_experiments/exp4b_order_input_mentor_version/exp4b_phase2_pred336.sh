#!/bin/bash
# =============================================================================
# Phase 2: Experiment 4b Legendre Delta in Value Matrix (Pred Len 336 only)
# Runs:     1 (pred_len 336) × 3 (seed 2021, 2022, 2023)
# =============================================================================

PROJECT_ROOT="/content/drive/MyDrive/Dist-Abl-PRL-All-Exs-ETTH1"
INFORMER_DIR="$PROJECT_ROOT/Informer2020-original"
LOG_DIR="$PROJECT_ROOT/logs/exp4b_phase2"
RESULTS_DIR="$PROJECT_ROOT/results"

mkdir -p "$LOG_DIR"
MASTER_LOG="$LOG_DIR/master_run_pred336.log"

echo "============================================================" | tee "$MASTER_LOG"
echo "Exp4b  |  PHASE 2 — RESULTS STABILITY (Pred Len 336 only)" | tee -a "$MASTER_LOG"
echo "============================================================" | tee -a "$MASTER_LOG"
echo "Seed      : 2021, 2022, 2023 | Pred lengths: 336"             | tee -a "$MASTER_LOG"
...
cp -r ../experiments/exp4b_order_input_mentor_version/models/* ./models/
...
for SEED in 2021 2022 2023; do
    for PRED_LEN in 336; do
        TOTAL=$((TOTAL + 1))
        RUN_ID="exp4b_ph2_ETTh1_legdelta_pred${PRED_LEN}_seed${SEED}"
        RUN_LOG="$LOG_DIR/${RUN_ID}.log"
        mkdir -p "$RESULTS_DIR/${RUN_ID}"

        echo "" | tee -a "$MASTER_LOG"
        echo "------------------------------------------------------------" | tee -a "$MASTER_LOG"
        echo "RUN $TOTAL/3: $RUN_ID"                                        | tee -a "$MASTER_LOG"
...
git checkout ./models/
# also clean up the extra file that git might leave untracked if we want, or just let git checkout handle the rest.
# To be safe and clean the extra file:
rm -f ./models/ordering_operator_positional.py
...
echo "" | tee -a "$MASTER_LOG"
echo "============================================================" | tee -a "$MASTER_LOG"
echo "PHASE 2 COMPLETE — RESULTS SUMMARY (Pred Len 336 only)"                     | tee -a "$MASTER_LOG"
